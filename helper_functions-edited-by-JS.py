...
def df_interpolate(orig_df, n, method='polynomial'):
    if n == 0:
        return orig_df
...
# Time heatmap
def time_heat_map(orig_df, show=True, sort=True, comp_var='Price', months=60, 
...
def keep_distinct_features(orig_df, comp_var='Price', corr_cut=0.7, look_back=7):
    ...
    for col_name1 in col_names:
        ...
        for col_name2 in names:
            m += 1
            print('Checking original feature {0} with {1} of {2} remaining '.format(n, m, num), end='                               \r' )
...
def hyper_param_opt(train_df, offset, interp=5,
    look_back_step=20, look_back_override=False, test_props=[.2,.15,.1],
    iter=10, model_type=Lasso, minmax_alpha=[.0005,.0015], n_alphas=10, 
    min_samples=24, min_test_points = 6, comp_var='Price'):
    ...
    for look_back in look_backs:
        ...
        for test_prop in test_props:
            if len(y)*test_prop >= min_test_points:
                ...
                print(str(len(y)*test_prop//1)+' test points is too few')
                num_tests -= 1
    hp_df = pd.DataFrame(data=results)
    return hp_df


def train_hopping_lasso(orig_df, comp_var='Price', interp=5, pred_len=60, iter=50,
    look_back_step=12, look_back_override=False, test_props=[.2,.15,.1],
    minmax_alpha=[.0005,.0015], n_alphas=10, model_type=Lasso):
    ...
    for offset in offsets:
        ...
        std_hp_df = full_hp_df.groupby(['offset','look_back','train_prop','neg_alpha']).std()
...
def hopping_lasso_predict(orig_df, best_hps, iter=100, comp_var='Price', interp=5):
    ...
    for hp_set in best_hps:
        ...
        slid_df_train = df_interpolate(slid_df_train, 5)
...
# Difference heatmap
def trainVtest_heat_map(train_df,test_df, include_mask=False, include_annot=False):
    """Create a heat map of the difference in Pearson correlation coefficients between all features for train vs test
    Positive numbers indicate an increase in correlation and negative numbers indicate a decrease with resepect to the train set
    ...
    tr_df = train_df.copy()
    te_df = test_df.copy()
    num_cols = tr_df.shape[1]

    # Compute correlation coefficient difference
    corr_diff = te_df.corr()-tr_df.corr()
    ...
    # Draw the heatmap with the mask and correct aspect ratio
    sns.heatmap(np.round(corr_diff,2), mask=mask, cmap=cmap, vmax=1, center=0,
                square=True, linewidths=0, annot=include_annot, annot_kws={"size": 7}, cbar_kws={"shrink":1})
    plt.show()
